...
- **GALLERY — a shared image-chain:** a top-level **GALLERY** view (toggled like a mode, next to MIL-SPEC) shows every published piece as a **hash-linked chain of blocks** — a tamper-evident "image-chain" (each block SHA-256-hashes its index + previous hash + CID + title/author, so altering any earlier block breaks every later hash). Data comes from **Pinata's pin list** for one **shared gallery account** (filtered by the keyvalue `gallery=hikali`), so it **auto-updates** when anyone publishes — no separate server. Publish from the SEAL tab via the opt-in **⛓ Publish to gallery** (title + handle), which tags the pin. Because files stay encrypted, blocks are **locked tiles** (title, author, short-CID, an on-the-fly QR, **→ Breach** to open it in the breach tool). Newest on top; the first block is the gold **genesis**. Everyone in the group uses the one shared gallery JWT.
...
- **图库 —— 共享图链：** 顶层 **图库** 视图（像模式一样切换，在 MIL-SPEC 旁边）把每件已发布作品显示为一条**哈希相连的区块链** —— 防篡改的「图链」（每个区块对 序号＋前一哈希＋CID＋标题／作者 做 SHA-256，改动任一早期区块都会让后续所有哈希断裂）。数据来自一个**共享图库账户**的 **Pinata 固定列表**（按 keyvalue `gallery=hikali` 过滤），所以有人发布时**自动更新**，无需独立服务器。在 SEAL 标签勾选 **⛓ 发布到图库**（标题＋代号）即可发布并打标签。由于文件保持加密，区块是**锁定的图块**（标题、作者、短 CID、即时二维码、**→ 破封** 直接载入破封工具）。最新在上，第一个区块是金色**创世块**。全组共用同一个图库 JWT。
