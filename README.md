...
- **IPFS (recommended host):** the **Fetch** field accepts a bare **CID**, an **`ipfs://`** URI, or any **gateway link** — IPFS gateways serve content with CORS, so the browser pulls **directly, no proxy** (unlike MEGA). It tries several gateways (`dweb.link`, `ipfs.io`, `w3s.link`, `gateway.pinata.cloud`, …) and uses the first that responds; since they all serve the same content-addressed bytes, fallback is safe and reliable. **Pin to IPFS in-app:** after sealing, each row has a **⬆ IPFS** button that pins the ciphertext via **Pinata** using *your own* JWT (paste or **scan it as a QR**; it's stored only in your browser's localStorage, never hardcoded). You get back a CID + a copyable gateway link to share. Free Pinata token at app.pinata.cloud → API Keys.
...
- **IPFS（推荐存储）：** **Fetch** 输入框支持裸 **CID**、**`ipfs://`** 地址或任意**网关链接** —— IPFS 网关带 CORS 提供文件，浏览器**直接拉取、无需代理**（不像 MEGA）。会依次尝试多个网关（`dweb.link`、`ipfs.io`、`w3s.link`、`gateway.pinata.cloud` 等），用第一个响应的；由于它们提供的是同一份内容寻址字节，回退安全可靠。**应用内固定到 IPFS：** 封印后每行有一个 **⬆ IPFS** 按钮，用*你自己*的 Pinata JWT（粘贴或**扫描二维码**载入；仅存于浏览器 localStorage，绝不硬编码）把密文固定到 IPFS，返回 CID 和可复制的网关链接。免费在 app.pinata.cloud → API Keys 获取令牌。
